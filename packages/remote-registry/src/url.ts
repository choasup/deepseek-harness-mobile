 ...
 * - `fp` 为服务器主机公钥指纹，格式固定为 `sha256:<base64>`。
 ...
 * formatRemoteUrl 共用这一套规则，保证两者互为逆运算，且
 * formatRemoteUrl 不会把一个非法的 RemoteMachine 静默序列化成一个
 * “看起来合法但语义已经变了”的 URL（例如端口越界被 WHATWG 的
 * `port` setter 静默丢弃）。
 */
function assertValidMachine(machine: RemoteMachine): void {
  ...
  try {
    new URL(`${REMOTE_URL_SCHEME}//${machine.host}/`)
  } catch {
...
export function formatRemoteUrl(machine: RemoteMachine): string {
  ...
  if (machine.port !== DEFAULT_SSH_PORT) url.port = String(machine.port)
